...
function draw_chart(data, p_div) {
  ...
  var pie = d3
    .pie() // Début et fin des angles des segments
    .value(function (d) {
      return d.count;
    }) // Extrait les nombres de chaque entrée de notre data
    .sort(null); // par default les valeurs sont triés par ordre descroissant on cancel ça en mettant a null

  // define tooltip
  var tooltip = d3.select(p_div).append("div").attr("class", "tooltip");
  ...
  var path = svg
    ...
    }) //Utilise le color scale pour definir chaque couleur de segments
    .each(function (d) {
      this._current - d;
    }); // animation sympa
  ...
  path.on("mouseover", function (d) {
    var total = d3.sum(
      dataset.map(function (d) {
        // calcul du nombre de segments de la data
